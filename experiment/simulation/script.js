...
// Close button add and automatic part 
function distance(cells, i, j, X2, X1) {
  Swal.fire({
    ...
    html:
      `<div style="position: relative; padding-top: 30px;">
         ...
         <h2 style="font-weight: 600; font-size: 1.4em; margin: 0; text-align:center;">
           Euclidean Distance (D) is computed using the formula √(X₂ − X₁)².
         </h2>
    ...
    didOpen: () => {
      ...
      // AUTO CALCULATION 
      let sub = X2 - X1;
      let actualDistance = Math.sqrt(Math.pow(sub, 2));

      const input = Swal.getInput();
      input.value = actualDistance;   // auto-fill
      input.readOnly = true;          //  (prevents editing)
    },

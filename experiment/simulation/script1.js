...
function distance(cells, i, j, X2, X1, Y2, Y1) {
  ...
  let sub1 = X2 - X1;
  let sub2 = Y2 - Y1;
  let add = Math.pow(sub1, 2) + Math.pow(sub2, 2);
  let actualDistance = Math.sqrt(add);
  let computedValue = actualDistance.toFixed(2);
  // ------------------------------------------
  ...
  Swal.fire({
    html: `
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2 style="font-weight: 600; font-size: 1.4em; margin: 0;">
         The Euclidean Distance (D) is computed  using the formula.
        </h2>
        <button id="closeBtn"
    ...
    `,
    input: "text",
    inputValue: computedValue,   // ✅ auto-filled value
    ...
    inputValidator: (value) => {
      if (!value) {
        return "Please provide the computed D value.";
      }

      if (Number(value) === Number(computedValue)) {
        ...
        cells[j].innerText = Number(computedValue);
        table.rows[j].cells[i].innerText = Number(computedValue);
        flag = true;
        ji++;
